...
export default function Home() { // Defines the main functional component, Home
  ...
  async function fetchArtworks() {
    ...
    setArtworks(data.data); // Updates the artworks state with the data array inside the `data` property
      ...
      artworks.forEach((artwork, i) => { // Iterates over each artwork item in the artworks array
        ...
        artworksList.push(
          <article key={i} className="artwork-article"> // Creates an article element for each artwork
            ...
            <p><strong>Artist:</strong> {artwork.artist_display}</p> // Displays the artist's name
            <p><strong>Date:</strong> {artwork.date_display}</p> // Displays the date of creation
